...
# 4) POST /sync_reonic_to_pipedrive – example: Reonic → Pipedrive sync
@router.post("/sync_reonic_to_pipedrive")
async def sync_reonic_to_pipedrive():
    """
    Mock sync that takes project data from Reonic and creates leads in Pipedrive.

    1) Fetch projects from Reonic.
    2) Normalize them into Pipedrive lead payloads.
    3) "POST" each payload to Pipedrive (still mocked locally).
    """
    if not PIPEDRIVE_API_TOKEN:
        raise HTTPException(status_code=400, detail="PIPEDRIVE_API_TOKEN is not set.")

    reonic_url = f"{REONIC_API_BASE}/projects"
    ...
    reonic_mock_payload = {
        ...
        "projects": [
            {
                "project_id": "REO-1001",
                "project_name": "Solar Site Hamburg",
                "estimated_value": 120000,
                "currency": "EUR",
                "owner_external_id": 11,
                "contact_external_id": 2101,
                "expected_go_live": "2025-02-01",
            },
            {
                "project_id": "REO-1002",
                "project_name": "Reonic EV Rollout Berlin",
                "estimated_value": 45000,
                "currency": "EUR",
                "owner_external_id": 18,
                "contact_external_id": 2109,
                "expected_go_live": "2025-03-15",
            },
        ],
    }
    ...
    projects = reonic_data.get("projects", [])
    ...
    for idx, project in enumerate(projects, start=1):
        payload = {
            "title": f"{project.get('project_name')} (Reonic)",
            "value": {
                "amount": project.get("estimated_value"),
                "currency": project.get("currency"),
            },
            "owner_id": project.get("owner_external_id"),
            "person_id": project.get("contact_external_id"),
            "expected_close_date": project.get("expected_go_live"),
            "visible_to": "1",
            "was_seen": False,
        }
        payload = {k: v for k, v in payload.items() if v is not None}

        mock_created_id = 2000 + idx
    ...
    return JSONResponse(
        content={
            "reonic_projects_count": len(projects),
